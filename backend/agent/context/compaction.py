"""Context compaction logic for agent runs.
...
from config.settings import TokenCounterStrategy

if TYPE_CHECKING:
    from agent.llm.client import AnthropicClient

_SCREENSHOT_PLACEHOLDER = "[screenshot captured]"
_ANCHOR_TRUNCATED_MARKER = "[original task truncated]"
_DEFAULT_PREVIEW_LENGTH = 500
_DEFAULT_RESULT_LENGTH = 1000
_DEFAULT_DIALOGUE_FALLBACK_CHARS = 12_000
_DEFAULT_CONTEXT_SUMMARY_MAX_CHARS = 32_000
_DEFAULT_RECONSTRUCT_TAIL_MESSAGES = 80
...
def _truncate_middle_chars(text: str, max_chars: int, marker: str) -> str:
    """Keep both ends of *text* while inserting *marker* in the middle."""
    if len(text) <= max_chars:
        return text
    if max_chars <= len(marker):
        return marker[:max_chars]
...
def _estimate_tokens(
    ...
    system_prompt: str = "",
    token_counter: TokenCounterStrategy | None = None,
) -> int:
    ...
    if token_counter is None:
        ...
        token_counter = get_settings().COMPACT_TOKEN_COUNTER
    estimate_text_tokens = _get_text_token_estimator(token_counter)
...
class Observer:
    ...
    def __init__(
        ...
        *,
        ...
        self._profile = profile or CompactionProfile(
            name="custom",
            token_budget=token_budget,
            token_counter="weighted",
            max_full_interactions=max_full_interactions,
            fallback_preview_chars=_DEFAULT_PREVIEW_LENGTH,
            fallback_result_chars=_DEFAULT_RESULT_LENGTH,
            summary_model=summary_model,
            max_full_dialogue_turns=max_full_dialogue_turns,
            dialogue_fallback_chars=_DEFAULT_DIALOGUE_FALLBACK_CHARS,
            context_summary_max_chars=_DEFAULT_CONTEXT_SUMMARY_MAX_CHARS,
            reconstruct_tail_messages=_DEFAULT_RECONSTRUCT_TAIL_MESSAGES,
            memory_flush=False,
        )
        self._max_full_interactions = self._profile.max_full_interactions
        self._max_full_dialogue_turns = self._profile.max_full_dialogue_turns
        self._token_budget = self._profile.token_budget
        self._client = claude_client
        self._summary_model = self._profile.summary_model or summary_model

    @property
    def profile(self) -> CompactionProfile:
    ...
    def should_compact(
        ...
        return (
            _estimate_tokens(
                messages,
                system_prompt,
                token_counter=self._profile.token_counter,
            )
            > self._token_budget
        )
        ...
        **Dialogue mode** (no ``tool_result`` blocks, typical DB replay):
        ...
        compacted_warm = tuple(
            _compact_message(
                ...
                preview_chars=self._profile.fallback_preview_chars,
                result_chars=self._profile.fallback_result_chars,
            )
    ...
    async def _compact_pure_dialogue(
        ...
        fb_chars = self._profile.dialogue_fallback_chars
        ...
        for _ in range(24):
            ...
            for i, msg in enumerate(msgs):
                ...
                if (
                    ...
                    break
            if not shrunk:
                anchor = _compact_anchor_message(msgs[0])
        ...
        if (
            ...
            for _ in range(24):
                if (
                    ...
                    return tuple(msgs)
                if self._shrink_previous_conversation_summary(msgs):
                    continue
                anchor = _compact_anchor_message(msgs[0])
                if anchor is not None:
                    msgs[0] = anchor
                ...
                msgs = [
                    msgs[0],
                    {
                        ...
                        + _truncate_transcript_chars(
                            ...
                            min(800, self._profile.dialogue_fallback_chars),
                        ),
                    },
                ]
                continue
    ...
    def _shrink_previous_conversation_summary(self, msgs: list[dict[str, Any]]) -> bool:
        """Shorten synthetic dialogue summaries in place."""
        for idx, msg in enumerate(msgs):
            content = msg.get("content")
            if (
                msg.get("role") != "assistant"
                or not isinstance(content, str)
                or not content.startswith("## Earlier conversation\n")
            ):
                continue
            body = content.split("\n", 1)[-1]
            if len(body) <= 40:
...
def _compact_anchor_message(message: dict[str, Any]) -> dict[str, Any] | None:
    ...
    This is a last-resort fallback when the original task itself prevents the
    compacted candidate from fitting under budget.
    """
    content = message.get("content")
    ...
    if isinstance(content, str):
        if len(content) <= 80:
            return None
        new_len = max(80, len(content) // 2)
        return {
            **message,
            "content": _truncate_middle_chars(
                content,
                new_len,
                f"\n{_ANCHOR_TRUNCATED_MARKER}\n",
            ),
        }

    if isinstance(content, list):
        flattened = _message_plain_text(message)
        if len(flattened) <= 80:
            return None
        new_len = max(80, len(flattened) // 2)
        return {
            **message,
            "content": _truncate_middle_chars(
                flattened,

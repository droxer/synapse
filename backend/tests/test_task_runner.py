...
@pytest.mark.asyncio
async def test_run_times_out_with_degrade_failure_mode_and_metrics(monkeypatch):
    monkeypatch.setattr(
    ...
    assert result.failure_mode == "degrade"
    ...
    assert agent_complete[0].data["failure_mode"] == "degrade"

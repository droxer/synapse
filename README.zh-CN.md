...
**对话驱动的任务执行** — 用户用自然语言描述任务，Synapse 会将每轮请求路由到单智能体 ReAct 循环或规划器管理的多智能体编排，并实时展示进度。

**沙盒化代码执行** — 智能体可以在按需创建的沙盒会话中编写和运行代码、安装软件包、查询数据库、自动化浏览器、操作文件并生成产物。

**多智能体规划** — 复杂任务会被自动分解为子任务，支持显式的计划声明。规划智能体协调多个并发运行的工作智能体，每个工作智能体拥有独立的沙盒环境。规划步骤实时跟踪和显示。

**可扩展的技能系统** — 技能是带 frontmatter 的 `SKILL.md` 包，可定义说明、可用工具、依赖和沙盒提示。Synapse 支持内置技能，也支持从 git URL、直接 URL、上传和远程注册表安装。

**持久化记忆与上下文压缩** — Synapse 会将用户级记忆注入提示，在长对话中按运行时策略压缩上下文，并支持频道场景下的事实检索。
...
**实时流式输出** — 前端通过 Server-Sent Events 实时渲染规划、工具调用、思考内容、产物、浏览器/计算机操作输出和子智能体进度。
...
- **执行路由** — `use_planner` 可强制启用规划模式；否则后端会按任务形态自动选择单智能体或规划器管理的执行方式
- **规划模式** — 显式的任务分解，包含步骤名称、执行类型和实时清单/进度跟踪
- **产物管理** — 沙盒中生成的文件可供下载和预览，并提供专用的资料库页面浏览所有产物
- **扩展思考** — 可配置的思考预算，用于复杂任务的深度推理
- **持久化记忆** — 用户级键值记忆、频道事实，以及用于长对话的运行时上下文压缩
...
| 前端 | Next.js 16, React 19, Tailwind CSS 4, Zustand, Framer Motion, Radix UI |
| 桌面 | Tauri v2, Rust, WKWebView (macOS) / WebView2 (Windows) |
| 沙盒 | Boxlite micro-VMs, E2B 云沙盒, provider-specific browser templates |
...
- [文档索引](docs/README.md) — 参考分片和深入文档，包括智能体运行时、记忆和评测
- [桌面应用指南](docs/desktop-app.md) — Tauri 桌面应用配置、OAuth 流程及故障排除
- [智能体记忆指南](docs/zh-CN/agent-memory-management.md) — 工作上下文、压缩、持久化记忆和事实
